...
# Parse arguments
parser = argparse.ArgumentParser(description="Run 5-fold MLM training with logging and saving.")
...
models_base_dir = os.path.join("models2", hp_tag)
summary_path = "hyperparamData/fiveFoldSummary.csv"
...
# Load dataset and assign folds deterministically by index
full_df = pd.read_csv("data/fullData/train_HLA_A0201.csv")
df = full_df[["peptide", "Length"]].copy().reset_index(drop=True)
df["fold"] = df.index % 5  # Folds 0 to 4
...
class LossLoggerCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        if logs is None:
            return
        if "loss" in logs:
            self.records.append({"step": state.global_step, "type": "train", "loss": logs["loss"]})
        if "eval_loss" in logs:
            self.records.append({"step": state.global_step, "type": "eval", "loss": logs["eval_loss"]})

early_stopping = EarlyStoppingCallback(early_stopping_patience=5)

# Store summary for all folds
all_fold_summaries = []
...
for fold in range(5):
    ...
    fold_dir = os.path.join(base_output, f"fold_{fold}")
    model_save_dir = os.path.join(models_base_dir, f"fold_{fold}")
    ...
    # Average losses per type from logs
    train_losses = [r["loss"] for r in loss_logger.records if r["type"] == "train"]
    eval_losses = [r["loss"] for r in loss_logger.records if r["type"] == "eval"]

    avg_train_loss = np.mean(train_losses) if train_losses else None
    avg_eval_loss = np.mean(eval_losses) if eval_losses else None

    print(f"Fold {fold} average train loss: {avg_train_loss}")
    print(f"Fold {fold} average eval loss: {avg_eval_loss}")
...
if os.path.exists(summary_path):
    ...
    print(f"Appended to existing {summary_path}")
else:
    summary_df.to_csv(summary_path, index=False)
    print(f"Created new {summary_path}")
